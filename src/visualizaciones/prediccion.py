...
def calcular_caracteristicas(texto):
    ...
    promedio_longitud_oracion = (
        np.mean([len(o.split()) for o in oraciones]) if oraciones else 0
    )
...
def render(st):
    ...
    st.write(
        "Esta herramienta predice si un resumen es **bueno o no**, basado en tu modelo entrenado."
    )
    ...
    if st.button("Predecir"):
        if user_input.strip() == "":
            ...
            if pred == 1:
                st.success(
                    f"✅ Es un **buen resumen** (probabilidad {proba[1]*100:.2f}%)"
                )
            else:
                st.error(
                    f"No parece un buen resumen (probabilidad {proba[0]*100:.2f}%)"
                )
            ...
            st.table(
                pd.DataFrame(
                    caracteristicas.items(), columns=["Característica", "Valor"]
                )
            )
